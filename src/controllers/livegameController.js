...
exports.joinLiveGame = catchAsync(async (req, res, next) => {
    ...
    const twoMinsToGameTime = +livegame.gameTime - 120000; //120000
...
exports.gameZone = catchAsync(async (req, res, next) => {
    ...
    if (currentTime >= twoMinsToGameTime && livegame.activeStatus && !livegame.gameInit) {
        ...
        if (currentTime < +livegame.gameTime) {
            ...
            // let objectForliveGame = {
            //   questions: livegame.questions.map((el) => el.question),
            //   options: livegame.questions.map((el) => el.options)
            // };
            const mapped = livegame.questions.map(q=> {
              const oneObject = {
                question:q.question,
                options: q.options
              }
            ...
            // console.log(+livegame.gameTime);
            // console.log(livegame.questions);
            // console.log(livegame.options);
            res.status(200).json({
                status: "success",
                // livegameQuestions: livegame.questions.map((el) => el.question),
                // livegameOptions: livegame.questions.map((el) => el.options),
                livegameQuestionAndOptions: mapped,
